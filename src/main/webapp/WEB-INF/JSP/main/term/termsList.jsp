                        ...
                        <c:forEach var="term" items="${terms }">
                            <c:choose>
                                <c:when test="${term.id  eq selectedTerm.id }">
                                    <option selected="selected" value="${term.id }">Семестр ${term.year }/${term.number }/ ${term.duration }</option>

                                </c:when>
                                <c:otherwise>
                                    <option value="${term.id }">Семестр ${term.year }/${term.number }/ ${term.duration }</option>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                    </select>
                ...
                <c:if test="${CURRENT_ROLE eq 1}">
                    <table cellspacing="0">
                        <form action="${CONTEXT }${CURRENT_MAPPING}/termCreating.php"
                              method="GET">
                            <tr>
                                <td style="padding-bottom: 20px; padding-top: 0">
                                    <input type="submit" id="button_long_name" value="Создать семестр...">
                                </td>
                            </tr>
                        </form>
                        ...
                        <tr>
                            <td style="padding-bottom: 20px">
                                <input type="button" onclick="modifyingTerm()" id="button_long_name"
                        ...
                        <tr>
                            <td style="padding-bottom: 20px">
                                <input type="button" id="button_long_name" value="Удалить текущий семестр..." onclick="deleteTerm()">
                            </td>
                        </tr>
                    </table>
                </c:if>
